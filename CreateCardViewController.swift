...
class CreateCardViewController: UIViewController, UITextFieldDelegate, ImagePickerDelegate, UINavigationControllerDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UIScrollViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        if let card = card {
            ...
            print(#line, "card.imgOffset:\(card.imgOffSet)")
...
//            print("scrollview.imgOffset:\(scrollView.contentOffset)")
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let card = self.card {
            card.team = teamTextField.text ?? ""
            card.name = nameTextField.text ?? ""
            card.photo = basePhoto.image
            card.frame = frameImage.image
            card.pngImage = UIImage.init(view: pngView)
//            card.backpngImage = card.backpngImage
        } else {
            ...
            let imgScale = CGFloat(0.5)
            let imgOffSet = CGPoint(x: 0, y: 0)
            
            card = Card(team: team, name: name, photo: photo, frame: frame, pngImage: pngImage, backpngImage: nil, position: nil, battingDirection: nil, throwingHand: nil, additionalInfo: nil, imgScale: imgScale, imgOffSet: imgOffSet)
            
           // card = Card(team: team, name: name, photo: photo, frame: frame, pngImage: pngImage, backpngImage: nil)
        }
    ...
    override func viewDidLayoutSubviews() {
        ...
        if viewDidLayoutSubviewsWasCalled == true {
            return
        }
        ...
        if let card = self.card {
            let offset = card.imgOffSet
            print(#line, offset, "card offset")
            scrollView.zoomScale = card.imgScale
            scrollView.setContentOffset(card.imgOffSet, animated: false)
            print(#line, scrollView.contentOffset, "scrollview offset")

//            scrollView.setZoomScale(card.imgScale, animated: false)
            print(#line, "scrollView.contentOffset: \(scrollView.contentOffset)")
...
//        print("\(String(describing: card?.imgOffSet.debugDescription))")
//    }
